...
### 8.1 Android 回声消除（AEC）无着落 —— 最高风险 ⚠️ 2026-08-20 真机实测：风险兑现

**已确认：Android 端没有可用 AEC，乐观假设不成立。** 真机（PHZ110）实测日志：

```
final=true text="你好啊。在吗？"
[useASR] Muted (TTS Active)
rms=0.07531 muted=true   rms=0.04945 muted=true   rms=0.01655 muted=true
rms=0.05984 muted=true   rms=0.07157 muted=true   rms=0.03100 muted=true
[BargeIn] 检测到插话
```

「静音」期间麦克风读数 `0.03~0.17`，是该设备真实底噪（`0.00013`）的
200~1000 倍——是喇叭声被麦克风直接收了回去，不是老人说话。打断在 AI 刚开口
的几百毫秒内触发，表现正是下面预判的「AI 刚开口就把自己掐断，反复循环」。

**已采纳的处理**：走下方补偿路径中的降级方案——`BARGE_IN.ENABLED` 改为
`false`，退回「不能打断」的老行为（`mobile/constants/BargeIn.ts`，
2026-08-20 提交）。方案 1（自研回声抑制）、方案 2（提高阈值 + ASR 文本
二次确认）留作后续；重新打开这个开关前必须先完成其中一个，不能直接改回
`true`——不然会复现这个死循环。

以下是当时按乐观假设推进时的原始记录，保留供参照：
...
### 8.2 双 AudioContext 采样率 ✅ 2026-08-20 真机实测：风险未兑现
...
**已确认：不需要这个退路。** 真机（PHZ110）实测多轮语音对话，ASR 与 TTS 的
AudioContext 在同一会话内交替/并存使用，用户确认「AI 说话的声音听起来正常」——
无变调、无语速异常。两个不同采样率的 context 在这个库的原生实现里可以正常共存，
不需要统一采样率或做重采样。
...
**2026-08-20 更新**：打断检测已因 §8.1 全局关闭（`BARGE_IN.ENABLED = false`，
web 与原生共用同一个常量），阈值表现这部分风险暂时不存在——两端现在都是「不能
打断」的老行为。其余播放/录音路径仍建议按 7.2 走一遍，确认 web 端没有退步。
